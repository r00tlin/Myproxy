...
class Crawler(object, metaclass=ProxyMetaclass):
    ...
    def crawl_gitproxylist(self):
        start_url = "https://raw.githubusercontent.com/fate0/proxylist/master/proxy.list"
        try:
            html = get_page(start_url)
            if html:
                find_ips = html.split("\n")
                for ip_port in find_ips:
                    try:
                        ip_port = json.loads(ip_port.replace("\n", ""))
                        yield "{}:{}".format(ip_port["host"], ip_port["port"])
                    except:
                        pass
        except: pass
